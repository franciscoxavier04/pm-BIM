    ...
    class SubmitOrCancelForm < ApplicationForm
      ...
      def back_link
        if @provider.new_record?
          OpenProject::StaticRouting::StaticRouter.new.url_helpers.openid_connect_providers_path
        else
          OpenProject::StaticRouting::StaticRouter.new.url_helpers.edit_openid_connect_provider_path(@provider)
        end
      end
    end
  end
