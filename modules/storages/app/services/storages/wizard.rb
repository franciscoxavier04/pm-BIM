...
module Storages
  class Wizard
    ...
    def prepare_next_step
      ...
      self.class.steps_by_name.fetch(next_step).prepare_on(self)

      next_step
    end

    def section_name(name)
      self.class.steps_by_name.fetch(name).section
    end
    ...
    def completed_steps
      steps.select { |name| self.class.steps_by_name.fetch(name).completed_on?(self) }
    end

    def pending_steps
      steps.reject { |name| self.class.steps_by_name.fetch(name).completed_on?(self) }
    end
  end
end
