    ...
    class SqlCollectionRepresenter
      ...
      class << self
        def ctes(walker_result)
          ...
          count = walker_result.filter_scope.count
          projection = if count == 0
                         # The following processes need the projection to return a non-empty response.
                         # If the count is 0, though, the projection would return nothing.
                         "SELECT 1"
                       else
                         walker_result.projection_scope
