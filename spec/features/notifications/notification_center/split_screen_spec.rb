...
RSpec.describe "Split screen in the notification center", :js do
  ...
  describe "basic use case" do
    ...
    it "can show the correct html title while opening and closing the split view" do
      ...
      first_title = "#{work_package.type.human_name}: #{work_package.subject} (##{work_package.id})"
      center.click_item notification
      ...
      second_title = "#{second_work_package.type.human_name}: #{second_work_package.subject} (##{second_work_package.id})"
      center.click_item notification
