...
RSpec.describe Relations::Scopes::Ghost do
  ...
  describe ".ghost" do
    context "when the user can see all work packages" do
      ...
      it "does not return any relations" do
        expect(Relation.ghost(user))
          .to be_empty
      end
    end
    ...
    context "when the user can see only the work packages in one project" do
      ...
      it "returns the relation within the other project" do
        expect(Relation.ghost(user))
          .to contain_exactly(relation2)
      end
    end
