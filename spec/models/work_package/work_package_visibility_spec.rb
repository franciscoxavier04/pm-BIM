...
RSpec.describe "WorkPackage-Visibility" do
  ...
  context "with public projects" do
    ...
    describe ".not_visible" do
      it "is viewable by anonymous, with the view_work_packages permission" do
        ProjectRole.anonymous.add_permission! :view_work_packages
        expect(WorkPackage.not_visible(anonymous)).to be_empty
      end
    end
  end
  ...
  context "with private projects" do
    ...
    describe ".not_visible" do
      it "is visible for the admin, even if the project is private" do
        expect(WorkPackage.not_visible(admin)).to be_empty
      end

      it "is visible for anonymous users, when the project is private" do
        expect(WorkPackage.not_visible(anonymous)).to contain_exactly(subject)
      end
    end
  end
end
