...
RSpec.describe OpenProject::MultiKeyHash do
  describe ".expand" do
    ...
    it "expands an array key into multiple keys" do
      expect(described_class.expand(%i[foo bar] => 2)).to eq({ foo: 2, bar: 2 })
    end

    it "expands multiple keys and values" do
    ...
    it "handles string keys" do
      expect(described_class.expand(["foo", "bar"] => 7)).to eq({ "foo" => 7, "bar" => 7 })
    end
