...
export class OpBaselineComponent extends UntilDestroyedMixin implements OnInit {
  ...
  public text = {
    ...
    time_description: (i:number) => {
      ...
      let formatted = DateTime.fromISO(`${date}T${time}${offset}`);
      formatted = formatted.setZone(this.userTimezone);

      const formattedDate = formatted.toLocaleString(this.timezoneService.getDateFormat());
      const formattedTime = formatted.toLocaleString(this.timezoneService.getTimeFormat());
  ...
  public ngOnInit():void {
    ...
    this.userOffset = DateTime.local().zone.name;
    this.resetSelection();
  ...
  public futureDateComparer():(dayElem:DayElement) => boolean {
    ...
    return (dayElem:DayElement) => DateTime.fromJSDate(dayElem.dateObj).day > now.day;
  }
