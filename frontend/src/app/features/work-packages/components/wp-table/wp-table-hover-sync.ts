...
export class WpTableHoverSync {
  ...
  private removeOldAndAddNewHoverClass(parentTableRow:Element | null, parentTimelineRow:Element | null) {
    ...
    const tableRow:HTMLTableRowElement = this.tableAndTimeline.querySelector(`tr.wp-row-${wpId}`)!;
    const timelineRow:HTMLDivElement = this.tableAndTimeline.querySelector(`div.wp-row-${wpId}`)
