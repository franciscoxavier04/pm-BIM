...
export class TimelineCellRenderer {
  ...
  public assignDateValues(
    ...
    this.assignDate(change, 'startDate', dates.startDate as DateTime);
    this.assignDate(change, 'dueDate', dates.dueDate as DateTime);
  ...
  protected displayDurationForDate(renderInfo:RenderInfo, date:DateTime):number {
    ...
    let duration = Number(Duration.fromISO(workPackage.duration || 'P1D').days.toFixed(0));
  ...
  getMarginLeftOfLeftSide(renderInfo:RenderInfo):number {
    ...
    start = _.isNaN(start.valueOf()) ? due : start;
  ...
  getMarginLeftOfRightSide(renderInfo:RenderInfo):number {
    ...
    start = _.isNaN(start.valueOf()) ? due : start;
    due = _.isNaN(due.valueOf()) ? start : due;

    const offsetStart = start.diff(renderInfo.viewParams.dateDisplayStart, 'days');
    const duration = due.diff(start, 'days').plus({ day: 1 });

    return calculatePositionValueForDayCountingPx(renderInfo.viewParams, offsetStart.plus(duration).days);
  }
  ...
  setElementPositionAndSize(element:HTMLElement, renderInfo:RenderInfo, start:DateTime, due:DateTime) {
    ...
    element.style.left = calculatePositionValueForDayCount(viewParams, offsetStart.days);

    // duration
    const duration = due.diff(start, 'days').plus({ day: 1 });
    element.style.width = calculatePositionValueForDayCount(viewParams, duration.days);
