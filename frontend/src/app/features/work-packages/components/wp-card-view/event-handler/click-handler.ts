...
export class CardClickHandler implements CardEventHandler {
  ...
  public handleEvent(card:WorkPackageCardViewComponent, evt:KeyboardEvent) {
  ...
  protected handleWorkPackage(card:WorkPackageCardViewComponent, wpId:any, element:HTMLElement, evt:KeyboardEvent) {
    this.setSelection(card, wpId, element, evt);

    card.itemClicked.emit({ workPackageId: wpId, double: false });
  }

  protected setSelection(card:WorkPackageCardViewComponent, wpId:string, element:HTMLElement, evt:KeyboardEvent) {
