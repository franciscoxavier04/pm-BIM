...
@Component({
  ...
  providers: [OpWorkPackagesCalendarService, OpCalendarService],
})
export class WorkPackagesCalendarComponent
  ...
  implements OnInit
{
  ...
  text = {
    cannot_drag_to_non_working_day: this.I18n.t(
      "js.team_planner.cannot_drag_to_non_working_day",
    ),
  ...
  ngOnInit(): void {
    ...
    this.wpTableFilters.hidden.push("project");
    this.calendar.resize$
      .pipe(this.untilDestroyed(), debounceTime(50))
      .subscribe(() => {
  ...
  public async calendarEventsFunction(
    ...
    await this.workPackagesCalendar.updateTimeframe(
      fetchInfo,
    ...
    if (this.alreadyLoaded) {
      ...
      this.workPackagesCalendar.currentWorkPackages$.subscribe(
        (collection: WorkPackageCollectionResource) => {
  ...
  public calendarMeetingsFunction(
    ...
    this.apiV3Service.meetings
      .filtered(filters, { pageSize: "-1" })
      .get()
      .subscribe((meetings) => {
        const events: EventInput[] = meetings.elements.map(
          (meeting: MeetingResource) => {
            const sameProject = this.currentProject.id === meeting.project.id;
            const title: string = sameProject
              ? meeting.title
              : `${meeting.project.name}: ${meeting.title}`;
            return {
              title,
              start: this.timezoneService
                .parseDatetime(meeting.startTime as string)
                .format(),
              end: this.timezoneService
                .parseDatetime(meeting.endTime as string)
                .format(),
              editable: false,
  ...
  private initializeCalendar() {
    const additionalOptions: { [key: string]: unknown } = {
      ...
      eventDidMount: (evt: CalendarViewEvent) => {
        ...
        const workPackage = event.extendedProps
          .workPackage as WorkPackageResource;
      ...
      eventResize: (resizeInfo: EventResizeDoneArg) => {
        ...
        const wp = resizeInfo.event.extendedProps
          .workPackage as WorkPackageResource;
        if (
          !wp.ignoreNonWorkingDays &&
          (this.weekdayService.isNonWorkingDay(start) ||
            this.weekdayService.isNonWorkingDay(due) ||
            this.workPackagesCalendar.isNonWorkingDay(start) ||
            this.workPackagesCalendar.isNonWorkingDay(due))
        ) {
      ...
      eventDrop: (dropInfo: EventDropArg) => {
        ...
        const wp = dropInfo.event.extendedProps
          .workPackage as WorkPackageResource;
        if (
          !wp.ignoreNonWorkingDays &&
          (this.weekdayService.isNonWorkingDay(start) ||
      ...
      eventDragStart: (dragInfo: EventDragStartArg) => {
        const wp = dragInfo.event.extendedProps
          .workPackage as WorkPackageResource;
        if (!wp.ignoreNonWorkingDays) {
          this.addBackgroundEventsForNonWorkingDays();
        }
      ...
      eventClick: (evt: EventClickArg) => {
        if (evt.event.extendedProps.meeting) {
          ...
          window.location.href = this.pathHelper.meetingPath(
        ...
        if (evt.event.extendedProps.workPackage) {
          const workPackageId = (
            evt.event.extendedProps.workPackage as WorkPackageResource
          ).id as string;
  ...
  public openContextMenu(event: MouseEvent): void {
    const eventContainer = (event.target as HTMLElement).closest(
      ".fc-event",
  ...
  private mapToCalendarEvents(workPackages: WorkPackageResource[]) {
    return workPackages.map((workPackage: WorkPackageResource) => {
      const startDate = this.workPackagesCalendar.eventDate(
        workPackage,
        "start",
      );
      ...
      return {
        ...
        durationEditable:
          this.workPackagesCalendar.eventDurationEditable(workPackage),
        end: exclusiveEnd,
        allDay: true,
        className: `fc-event-clickable __hl_background __hl_type_${workPackage.type.id || ""}`,
        workPackage,
      };
    });
  }
  ...
  private handleDateClicked(info: DateSelectArg) {
    ...
    const nonWorkingDays =
      this.weekdayService.isNonWorkingDay(info.start) ||
      this.weekdayService.isNonWorkingDay(due) ||
      this.workPackagesCalendar.isNonWorkingDay(info.start) ||
      this.workPackagesCalendar.isNonWorkingDay(due);
    ...
    void this.$state.go(splitViewRoute(this.$state, "new"), {
      defaults,
  ...
  private addBackgroundEventsForNonWorkingDays() {
    addBackgroundEvents(
      ...
      (date) =>
        this.weekdayService.isNonWorkingDay(date) ||
        this.workPackagesCalendar.isNonWorkingDay(date),
    );
  }
}
