...
module Projects::Exports::PDFExport
  module TableOfContent
    ...
    def build_toc_data_list_entry(project, info_map) # rubocop:disable Metrics/AbcSize
      level_path = info_map[project.id][:level_path]
      level = [level_path.length, styles.toc_max_depth].min
      level_style = styles.toc_item(level)
      level_string = "#{level_path.join('.')}. "
      page_nr_string = (info_map[project.id][:page_number] || "000").to_s
      { id: project.id,
        level_string:,
